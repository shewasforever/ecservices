<?php
namespace GetProductInfoFromEC\Controller;//获取商品图片接口
    	...
    	if($client_accesstoken){
    		if($server_accesstoken !== $client_accesstoken){
    			...
    			exit($row_json);
			 	...
			 	$Goods = M("Goods");
                $condition['hebei_p_id'] = array(NEQ,'');
                $arr =  $Goods->field("hebei_p_id,goods_name")->where($condition)->select();

               
				$row["productInfoList"] = $arr;
                $row["returnMsg"] = "商品映射关系信息";
                $row["isSuccess"] = true; 
                echo '<pre>';
                print_r($row);
                echo '</pre>';
